    ...
    loop {
        ...
        match stream_completion_with_retry(provider, &snapshot, tools, &tx, cancel.clone()).await {
            Ok(outcome) => {
                ...
                // Only auto-continue if there's actually something wrong:
                // either no output at all, or output that looks truncated
                // (non-empty but stream didn't end cleanly).
                if calls.is_empty() && auto_continues < MAX_AUTO_CONTINUE && (no_output || (!clean && !text.trim().is_empty())) {
                    auto_continues += 1;
                ...
                // After each tool, check if the user cancelled so we can
                // exit immediately instead of starting a new turn.
                if cancel.load(Ordering::SeqCst) {
                    let _ = tx.send(StreamEvent::Done).await;
                    return;
                }
